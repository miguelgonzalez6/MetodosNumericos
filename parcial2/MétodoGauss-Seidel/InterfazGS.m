function varargout = InterfazGS(varargin)
% INTERFAZGS MATLAB code for InterfazGS.fig
%      INTERFAZGS, by itself, creates a new INTERFAZGS or raises the existing
%      singleton*.
%
%      H = INTERFAZGS returns the handle to a new INTERFAZGS or the handle to
%      the existing singleton*.
%
%      INTERFAZGS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in INTERFAZGS.M with the given input arguments.
%
%      INTERFAZGS('Property','Value',...) creates a new INTERFAZGS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before InterfazGS_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to InterfazGS_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help InterfazGS

% Last Modified by GUIDE v2.5 28-Oct-2016 07:50:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @InterfazGS_OpeningFcn, ...
                   'gui_OutputFcn',  @InterfazGS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before InterfazGS is made visible.
function InterfazGS_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to InterfazGS (see VARARGIN)

% Choose default command line output for InterfazGS
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes InterfazGS wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = InterfazGS_OutputFcn(hObject, eventdata, handles) 
